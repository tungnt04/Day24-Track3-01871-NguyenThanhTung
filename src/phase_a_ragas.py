...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    for a, pq in zip(answers, per_q):
        results.append(RagasResult(
            ...
            faithfulness=_f(pq.faithfulness),
            answer_relevancy=_f(pq.answer_relevancy),
            context_precision=_f(pq.context_precision),
            context_recall=_f(pq.context_recall),
        ))
    return results
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    # avg_score theo distribution — dùng để phá thế hoà (distribution kém hơn thắng)
    dist_avg = {
        d: (sum(r.avg_score for r in results if r.distribution == d) /
            max(1, sum(1 for r in results if r.distribution == d)))
        for d in ["factual", "multi_hop", "adversarial"]
    }
    dominant_dist = max(
        ["factual", "multi_hop", "adversarial"],
        key=lambda d: (sum(matrix[m][d] for m in matrix), -dist_avg[d]),
    )
